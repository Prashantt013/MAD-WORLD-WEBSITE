...
const shelves = [
  { label: 'Hall of Fame', href: '/hall-of-fame', items: getHallOfFame().slice(0, 8) },
  { label: 'Games', href: '/games', items: ALL_TITLES.filter((title) => title.category === 'games').slice(0, 8) },
  { label: 'Anime', href: '/anime', items: ALL_TITLES.filter((title) => title.category === 'anime').slice(0, 8) },
  { label: 'Shows', href: '/shows', items: ALL_TITLES.filter((title) => title.category === 'shows').slice(0, 8) },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    setRecommendation(ALL_TITLES[Math.floor(Math.random() * ALL_TITLES.length)]);
    setQuote(getRandomQuote() || quotes[0]);
  }, []);
  ...
  return (
    <main>
      ...
      <section className="container home-section"><div className="section-heading"><div><span className="eyebrow">Your queue</span><h2>Continue watching</h2><p>The worlds currently in rotation.</p></div><Link href="/library" className="text-link">View library →</Link></div><div className="shelf-row">{ALL_TITLES.filter((title) => title.category !== 'games' && title.up_to_date).slice(0, 6).map((title) => <CaseCard key={title.id} title={title} compact />)}</div></section>
